...
export class MagicWords {
    ...
    private dialogueData: any = null;
    private currentDialogueIndex: number = 0;
    private dialogueContainer: any = null;
    ...
    private nextDialogue(): void {
        if (!this.dialogueData || !this.dialogueData.dialogue) return;
    ...
    private processDialogueText(text: string): string {
        // Replace emotion tags with actual emojis
        let processedText = text;
        
        if (this.dialogueData.emojies) {
            this.dialogueData.emojies.forEach((emoji: any) => {
    ...
    private createCharacterAvatar(characterName: string): void {
        if (!this.dialogueData.avatars) return;
        
        const avatar = this.dialogueData.avatars.find((a: any) => a.name === characterName);
        if (!avatar) return;
