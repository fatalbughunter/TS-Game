export class Menu {
    constructor(app, onStartGame, onMagicWords, onPhoenixFlame) {
        this.app = app;
    ...
    updateTitlePositions(dimensions) {
        const isMobile = dimensions.width < 768;
        const isTablet = dimensions.width >= 768 && dimensions.width < 1025;
        // Update SOFTGAMES title
        const softGamesTitle = this.container.children.find((child) => child.children && child.children.some((c) => c.text === 'SOFTGAMES'));
        if (softGamesTitle) {
        ...
        const mainTitle = this.container.children.find((child) => child.text === 'ACE OF SHADOWS');
        if (mainTitle) {
        ...
        const magicWords = this.container.children.find((child) => child.text === 'MAGIC WORDS');
        if (magicWords) {
        ...
        const phoenixFlame = this.container.children.find((child) => child.text === 'PHOENIX FLAME');
        if (phoenixFlame) {
